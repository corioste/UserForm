...
export class UserLocalStorageRepositoryImpl implements UsersRepository {
    async GetUser(): Promise<User[]> {
        ...
        db.collection(STORAGE_NAME).add({sample:"1"})
        console.log(dataRef)
    ...
    async CreateUser(data: User) {
        ...
        db.collection(STORAGE_NAME).add({
            id:data.id,
    ...
    async UpdateUser(data: User) {
        ...
        var addressbookRef = db.collection(STORAGE_NAME).doc(data.id)
    ...
    async DeleteUser(data: User) {
        ...
        db.collection("addressbook").doc(data.id).delete().then(() => {
