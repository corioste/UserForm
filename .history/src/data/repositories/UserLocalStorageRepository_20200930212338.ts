...
export class UserLocalStorageRepositoryImpl implements UsersRepository {
    ...
    GetUser(): Promise<User[]> {
        ...
        console.log(res)
        return res.map((user: UserDTO) => new User(user.id, user.firstName, user.middleName, user.lastName, user.dateOfBirth, user.gender, user.email));
    }
    ...
    SelectUser(data: User): User {
        ...
        var jsonString = localStorage.getItem("userList");
        var res = JSON.parse(jsonString || '[]');
        var firstName, middleName, lastName, dateOfBirth, email,gender, id;
        for (let i = 0; i < res.length; i++) {
            if (intData === res[i].id) {
