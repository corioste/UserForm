...
export class UserLocalStorageRepositoryImpl implements UsersRepository {
    async GetUser(): Promise<User[]> {
        ...
        return data.map((user: UserDTO) => new User(user.id, 
                                                    user.firstName, 
                                                    user.middleName, 
                                                    user.lastName, 
                                                    user.gender, 
    ...
    async CreateUser(data: any): Promise<any> {
        ...
        async function uploadTaskPromise() {
            return new Promise(function (resolve, reject) {
                var uploadTask = storage.ref(`images/${data.data.image.name}`).put(data.data.image);

                uploadTask.on('state_changed',
                    (snapshot: any) => { },
                    (error: any) => { },
                    () => {
                        storage.ref('images').child(data.data.image.name).getDownloadURL().then((url: any) => {
                            imageURL = url;
                            userRef
                                .doc(data.data.id)
                                .set({
