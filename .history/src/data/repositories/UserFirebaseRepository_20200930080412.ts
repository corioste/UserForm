...
export class UserLocalStorageRepositoryImpl implements UsersRepository {
    ...
    async SelectUser(data: User): Promise<User> {
        ...
        for (let i = 0; i < itemArray.length; i++) {
            if (intData === itemArray[i].id) {

                firstName = itemArray[i].firstName;
                middleName = itemArray[i].middleName;
                lastName = itemArray[i].lastName;
                dateOfBirth = itemArray[i].dateOfBirth;
                email = itemArray[i].email;
                id = itemArray[i].id;
